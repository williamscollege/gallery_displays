<?php
	...
	Db_Linked is a basic root class for objects that are tied to a DB record - e.g. users, eq_groups, eq_items, etc.
	It provides simple functions to select, insert, and update the DB record associated with the object.
	NOTE: it does NOT support a delete method, and it does NOT load associations (e.g. group membership).
	If that kind of functionality is needed it should be implemented in a sub-class (for now, anyway, we're trying to keep this pretty streamlined).
	...
	Objects also have a field called matchesDb, which indicates whether the values stored in the object match the values stored in the corresponding database record.
	When a new object is created matchesDb is always false;
	...
	This first parameter is a search hash - i.e. a hash where the keys are the names of the fields and the values are used to create conditions of the
	select statement that retrieves the record from the DB.
	...
	NOTE: for either single-load approach, if the data has no matching record then the matchesDb attribute of the object will be false - check that attribute in your code before relying on the object!
	...
	The object has a method updateDb which persists the object data in the DB. If there's already a record for the object, then that record is updated.
	If there is NOT a record, then a new one is inserted.
