<?php

	$LANGUAGE['en'] = [
		'app_short_description'          => 'Create or manage a signup sheet with openings at specific times.',
		'app_sign_in_action'             => 'Sign in',
		'app_signed_in_status'           => 'Signed in',
		'app_sign_in_msg'                => 'To sign in, please use your ' . LANG_INSTITUTION_NAME . ' username and password.',

		'home'                           => 'home',
		'go_to'                          => 'go to....',
		'search'                         => 'search',
		'prompt_select'                  => '-- choose from the list --',
		'show_hide'                      => 'show/hide',
		'mark_for_delete'                => 'mark this for removal - the actual removal occurs on update',
		'unmark_for_delete'              => 'undo the mark for removal',
		'used_by'                        => 'used by',

		'msg_do_sign_in'                 => 'Please sign in.',
		'msg_failed_sign_in'             => 'Sign in failed.',
		'msg_record_missing'             => 'Record does not exist in database',
		'msg_record_duplicate'           => 'Record already exists in database',
		'msg_missing_parameter'          => 'Required information is missing',

		'msg_lti_failed_authentication'  => 'Oops! The LTI application failed to authenticate. For help, please contact ITech.',
		'msg_lti_abort_student_view'     => '<p>Sorry, the "Student View" is not available for Signup Sheets.</p><p>(This is because each user has distinct signups and there is no way to generalize those into a single "Student View".)</p>',
		'msg_lti_cannot_set_cookie'      => 'Oops! The LTI application was unable to set a required cookie. For help, please contact ITech.',

		'alert_error'                    => 'Error!',
		'alert_success'                  => 'Success!',
		'alert_info'                     => 'Note:',

		'username'                       => 'Username',
		'password'                       => 'Password',

		'you_possesive'                  => 'your',
		'owned_by'                       => 'owned by',
		'attribution'                    => 'by',
		'public'                         => 'public',

		'created_at'                     => 'created',
		'updated_at'                     => 'last updated',

		'published_true'                 => 'public',
		'published_false'                => 'private',
		'verified_true'                  => 'verified',
		'verified_false'                 => 'not verified',
		'active_true'                    => 'usable',
		'active_false'                   => 'inactive',
		'active'                         => 'usable',

		'not'                            => 'not',

		'view'                           => 'view',
		'edit'                           => 'edit',
		'update'                         => 'update',
		'save'                           => 'save',
		'create'                         => 'create',
		'delete'                         => 'delete',
		'publish'                        => 'publish',
		'published'                      => 'published',
		'verify'                         => 'verify',
		'verified'                       => 'verified',
		'cancel'                         => 'cancel',

		'no_permission'                  => 'You do not have permissions to do that.',
		'not_supported'                  => 'That is not supported',
		'invalid_value'                  => 'invalid value',
		'nothing_from_the_list'          => 'nothing in the list',
		'invalid_string_base_chars_only' => 'only A-Z, a-z, and 0-9 allowed',

		# Terms below are specific to this project: Signup Sheets
		'sheet_openings_all'             => 'Available Openings',
		'sheet_openings_signup'          => 'Available Openings',
		'signups_all'                    => 'Signups',
		'sheets_all'                     => 'Sheets',
		'sheets_edit_one'                => 'Edit My Sheet',
		'add_sheet'                      => 'Add Sheet',
		'my_account'                     => 'Preferences',
		'help'                           => 'Sheets Help',
		'calendar'                       => 'Calendar',
		''                               => ''
	];